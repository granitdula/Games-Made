# Tilemap Demo
# Fix issue with bullet collision being to early.
import pygame as pg
import sys
...
class Game:
    ...
    def load_data(self):
        ...
        self.dim_screen.fill((0, 0, 0, 180)) # 180 is alpha channel between 0 and 255 where 255 is opaque.
    ...
    def new(self):
        # initialise all variables and do all the setup up for the game.
        ...
        for tile_object in self.map.tmxdata.objects:
            # There is a dictionary containing properties of each object.
            ...
            if tile_object.name in ['health', 'shotgun', 'machine_gun']:
    ...
    def update(self):
        ...
        for hit in hits:
            ...
            if hit.type == 'shotgun':
                ...
                self.player_img = pg.image.load(path.join(self.img_folder, PLAYER_IMG)).convert_alpha()
            if hit.type == 'machine_gun':
                hit.kill()
                self.effects_sounds['gun_pickup'].play()
                self.player.weapon = 'machine_gun'
                self.player_img = pg.image.load(path.join(self.img_folder, PLAYER_IMG2)).convert_alpha()
        # mobs hit player
    ...
    def show_start_screen(self):
        ...
        self.draw_text("ZOMBIE GAME", self.title_font, 100, GREEN,
